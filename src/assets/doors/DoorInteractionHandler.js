import { VirtualKeypad } from './PasscodeInput.js';
...
export class DoorInteractionHandler {
  ...
  showPasscodeUI(door) {
    ...
    this.passcodeUI = new VirtualKeypad(
      ...
      null, // doorPosition (not used by VirtualKeypad)
      door.passcode ? door.passcode.length : 4 // expected password length
    );
  ...
  forceCameraLookAtDoor() {
    // VirtualKeypad doesn't need camera positioning
  }
  ...
  update(delta) {
    // VirtualKeypad doesn't need updates
