...
export class DoorBase {
  constructor(scene, physicsWorld, position, options = {}) {
    try {
      console.log('DoorBase constructor called with position:', position, 'options:', options);
      this.scene = scene;
  ...
  _updatePhysicsBody() {
    ...
    if (!this.body) {
      try {
        ...
        // Create kinematic body directly
        const shape = new CANNON.Box(new CANNON.Vec3(collisionSize[0] / 2, collisionSize[1] / 2, collisionSize[2] / 2));
        this.body = new CANNON.Body({
          mass: 0,
          type: CANNON.Body.KINEMATIC,
          material: this.physicsWorld.materials.wall || this.physicsWorld.materials.ground
        });
        this.body.addShape(shape);
        this.body.position.set(collisionCenter.x, collisionCenter.y, collisionCenter.z);
        this.physicsWorld.world.addBody(this.body);
        
        // Add to static bodies collection for proper tracking
        if (this.physicsWorld.staticBodies) {
          this.physicsWorld.staticBodies.add(this.body);
        }
  ...
  canPlayerInteract(playerPosition) {
    // Use larger interaction distance when door is open, smaller when closed
    const currentInteractionDistance = this.isOpen ? this.interactionDistance : 8.0;

    if (!this.isOpen) {
      // When door is closed, use simple bounds around the door frame
      const doorMinX = this.mesh.position.x - this.size[0] / 2;
      const doorMaxX = this.mesh.position.x + this.size[0] / 2;
      const doorMinY = this.mesh.position.y;
      const doorMaxY = this.mesh.position.y + this.size[1];
      const doorZ = this.mesh.position.z;
      ...
      const zDistance = Math.abs(playerPosition.z - doorZ);

      const canInteract = withinXBounds && withinYBounds && zDistance <= currentInteractionDistance;

      if (zDistance < 15 || withinXBounds) {
        console.log('Door interaction check (closed):', {
          playerPos: playerPosition,
          doorPos: this.mesh.position,
          doorBounds: { minX: doorMinX, maxX: doorMaxX, minY: doorMinY, maxY: doorMaxY, z: doorZ },
          withinXBounds, withinYBounds, zDistance, canInteract
        });
      }
      ...
      // Create bounds around the door panel (with some padding)
      const panelMinX = panelWorldPos.x - this.size[0] / 2 - 1; // Extra padding
      const panelMaxX = panelWorldPos.x + this.size[0] / 2 + 1;
      ...
      const zDistance = Math.abs(playerPosition.z - panelZ);
      ...
      if (zDistance < 15 || withinXBounds) {
        console.log('Door interaction check (open):', {
          ...
          panelBounds: { minX: panelMinX, maxX: panelMaxX, minY: panelMinY, maxY: panelMaxY, z: panelZ },
          withinXBounds, withinYBounds, zDistance, canInteract
        });
      }
