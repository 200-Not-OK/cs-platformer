...
export class PhysicsWorld {
  constructor(scene = null) {
    ...
    // Door manager for handling door interactions
    this.doorManager = new DoorManager(this.scene, this);
  ...
  setDoorManager(gameInstance) {
    if (this.doorManager && gameInstance) {
      this.doorManager.gameInstance = gameInstance;
      this.doorManager.interactionHandler = new DoorInteractionHandler(gameInstance);
      console.log('� Door manager gameInstance set');
    } else {
      console.warn('Cannot set door manager gameInstance: missing doorManager or gameInstance');
    }
  }
  ...
  createDoorBody(position = new THREE.Vector3(), size = [2, 4, 0.2]) {
    // Use box shape for doors - kinematic so they can move but don't respond to forces
    const halfExtents = new CANNON.Vec3(size[0] * 0.5, size[1] * 0.5, size[2] * 0.5);
    const shape = new CANNON.Box(halfExtents);

    const body = new CANNON.Body({
      mass: 0, // Zero mass for kinematic body
      material: this.groundMaterial,
      type: CANNON.Body.KINEMATIC // Kinematic bodies can move but don't respond to forces
    });
  ...
  step(deltaTime) {
    ...
    // Update door manager if available
    if (this.doorManager) {
      this.doorManager.update(clampedDelta);
    }
